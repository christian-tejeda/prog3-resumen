...
* Tenemos $n$ pedidos etiquetados $1, \dots, n$, con cada pedido $i$ especificando un tiempo de comienzo $s_{i}$ y un tiempo de finalización $f_{i}$. Cada intervalo $i$ tiene un _valor_ $v_{i}$
...
* Un pedido $i$ viene _antes_ de un pedido $j$ si $i < j$.
...
* Implementamos esta estrategia en el procedimiento $\texttt{M-Compute-Opt}$ que hace uso de un array $M[0 \dots n]$; $M[j]$ comienza vacío y contendrá el valor de $\texttt{Compute-Opt}(j)$ cuando sea calculado. Para determinar $\textrm{OPT}(n)$ llamamos a $\texttt{M-Compute-Opt}(n)$.
